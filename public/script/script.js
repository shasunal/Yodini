document.addEventListener("DOMContentLoaded", () => {
  ...
  deck.forEach((tarot, index) => {
    // adds flip animation to all tarot cards
    // MIGHT MOVE TO CSS
    ...
    cards.set(tarot, {
      // flip animation: ISSUE:: is pretty hardcoded translation & doesn't exactly align with the card-stack
...
//mouse entering and leaving the cards
    tarot.addEventListener("mouseenter", () => {
      const quote = tarot.dataset.quote;

      if (tarot.classList.contains("flipped")) {
        console.log("Hovered card index:", index);
        console.log("Quote from dataset:", quote);
        if (quotesDiv && quote) {
    ...
    tarot.addEventListener("mouseleave", () => {
      if (tarot.classList.contains('flipped')&&quotesDiv){
    ...
    tarot.addEventListener("click", () => {
      if (!tarot.classList.contains('flipped') ){
        // users can only flip cards once
        cards.to(tarot, {
          x: `0px`,
          rotateY: 0,
        });
        ...
        if (index == 0 && line1) {
          ...
          ticketCont.to(ticket, {
            // animate continue ticket
            delay: 0.6,
            x: "0px",
          });
        }
